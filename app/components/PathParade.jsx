...
export default function PathParade() {
  ...
  useLayoutEffect(() => {
    const ctx = gsap.context(() => {
      ...
      const dots = gsap.utils.toArray(".mp-dot"); // 6 dots

      // Centro geométrico dos dots
      gsap.set(dots, { xPercent: -50, yPercent: -50 });

      // Distribui os dots igualmente no path usando 'start' com função:
      // cada dot começa deslocado (offset) ao longo da curva.
      gsap.to(dots, {
        duration: 8, // velocidade do loop
        repeat: -1,
        ease: "none",
        motionPath: (i) => ({
          path,
          align: path,
          alignOrigin: [0.5, 0.5],
          autoRotate: true, // pode ser um número p/ offset angular (ex.: 90)
          start: i / dots.length, // desloca início de cada dot
          end: 1 + i / dots.length, // garante que todos percorram 100%
        }),
        // Truque: cada dot anda o ciclo completo mas com fase diferente.
      });
    }, wrapRef);
  ...
  return (
    ...
    >
      ...
      {[...Array(6)].map((_, i) => (
        <div
          key={i}
          className="mp-dot absolute z-10 pointer-events-none"
          style={{
            width: 10,
            height: 10,
            borderRadius: "999px",
            background:
              "radial-gradient(circle, rgba(255,255,255,.95) 0 40%, rgba(128,0,128,.9) 40% 100%)",
            boxShadow: "0 0 10px rgba(128,0,128,.6)",
          }}
        />
      ))}

      {/* Seu SVG. Dê id="motionPath" no path que servirá de trilha */}
      <svg viewBox="0 0 991 112" className="w-full h-full">
        <defs>
