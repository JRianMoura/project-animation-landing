...
export default function LogoPathDrawLoop() {
  const wrapRef = useRef(null);

  useLayoutEffect(() => {
    const ctx = gsap.context(() => {
      const path =
        wrapRef.current.querySelector("#motionPath") ||
        wrapRef.current.querySelector(".svg-elem-10");

      const len = path.getTotalLength();

      // Estado inicial: nada visível (0 de traço, resto é "gap")
      gsap.set(path, {
        strokeDasharray: `0 ${len}`,
        strokeDashoffset: 0,
      });

      //   gsap.set(path, { strokeDasharray: `60 ${len}`, strokeDashoffset: 0 }); // 60px visíveis

      // Duração baseada no tamanho do path (px/seg), pra ficar natural em paths diferentes
      const seconds = len / 240; // ajuste esse "240" (maior = mais lento)
      ...
      // Cresce do zero até 100% do comprimento
      tl.to(path, { strokeDasharray: `${len} ${len}`, duration: seconds });

      // Pequena pausa “dramática”
      tl.to({}, { duration: 0.4 });

      // Reset instantâneo para recomeçar
      tl.set(path, { strokeDasharray: `0 ${len}`, strokeDashoffset: 0 });
    }, wrapRef);
  ...
  return (
    ...
    >
      {/* Dica visual extra (opcional): glow suave no stroke */}
      <svg
        viewBox="0 0 991 112"
      ...
      >
        <defs>
          ...
          {/* gradiente opcional pra deixar o traço mais vivo */}
        ...
        {/* Use o SEU path (o mesmo que você está usando). 
            Se quiser, adicione id="motionPath" nele. */}
        <path
          id="motionPath"
          ...
          fill="none"
          stroke="url(#grad)" // pode trocar por "#800080"
          strokeWidth="4"
          strokeLinecap="round"
          filter="url(#neon)"
        />
